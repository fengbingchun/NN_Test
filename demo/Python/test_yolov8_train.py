...
def train(task, yaml, epochs):
	...
	results = model.train(data=yaml, epochs=epochs, imgsz=640, patience=50) # train the model, supported parameter reference, for example: runs/segment(detect)/train3/args.yaml
	...
	# model.export(format="engine", imgsz=640, dynamic=False, verbose=False, batch=1, workspace=2, half=True) # tensorrt fp16
	# model.export(format="engine", imgsz=640, dynamic=False, verbose=False, batch=1, workspace=2, int8=True, data=yaml) # tensorrt int8
